...
public class IMSController {
    ...
    public void requestTextUpdateFromInputEvent(InputConnection ic) {
        ...
        try {
            ...
            if (ei != null && tn.eluea.kgpt.BuildConfig.APPLICATION_ID.equals(ei.packageName)) {
                return;
            }
        } catch (Throwable ignored) {}
        if (ic == null) {
        ...
        inputEventHandler.postDelayed(() -> {
            ...
            // Prefer getTextBeforeCursor: it works in most editors even when getExtractedText() returns null.
            try {
                CharSequence before = current.getTextBeforeCursor(INPUT_EVENT_MAX_BEFORE, 0);
                if (before != null) {
                    typedText = before.toString();
